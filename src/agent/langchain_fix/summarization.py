...
# Import memory storage function
from src.agent.memory import add_memory
...
{summary}
    ...
    def _store_to_memory(
        ...
        """Store messages to mem0 memory for long-term retrieval.

        This is a non-blocking operation that stores the original uncompressed
        messages to mem0 before they are summarized. Failures are logged but
        do not affect the main summarization flow.

        Args:
            messages: Filtered messages to store (excluding previous summary messages).
            timestamp: ISO format timestamp of when summarization occurred.
        """
        if not messages:
            return

        try:
            # Format messages for mem0 storage with metadata
            formatted_messages = []
            for msg in messages:
                msg_data = {
            ...
            # Store to mem0
            add_memory(formatted_messages)
            logger.debug("Stored %d messages to mem0 memory", len(messages))
        except Exception as e:  # noqa: BLE001
            # Non-blocking: log error but don't affect main flow
            logger.warning(
                "Failed to store messages to mem0 memory (%d messages): %s: %s",
                len(messages),
                type(e).__name__,
                e,
            )
    ...
    def _offload_to_backend(
        ...
        # Store messages to mem0 memory (non-blocking, failures are logged but don't affect main flow)
        self._store_to_memory(filtered_messages, timestamp)
